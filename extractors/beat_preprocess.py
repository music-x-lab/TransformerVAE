...
def get_framed_subbeat_annotation(entry,proxy_name,beat_division):
    n_frame=entry.n_frame
    delta_time=entry.prop.hop_length/entry.prop.sr
    beat=np.array(entry.dict[proxy_name].get(entry))
    beat_time=beat[:,0]
    beat_interp=np.linspace(beat_time[:-1],beat_time[1:],beat_division+1).T
    beat_frame_interp=np.round(beat_interp/delta_time).astype(np.int)
    beat_pos=np.round(beat[:,1]).astype(np.int)
    result=np.zeros((n_frame,),dtype=np.float32)
    for i in range(len(beat)-1):
        for j in range(beat_division):

            if(beat_frame_interp[i,j]>=0 and beat_frame_interp[i,j]<n_frame):
                result[beat_frame_interp[i,j]]=(beat_pos[i]-1)*beat_division+j+1
    if(beat_frame_interp[-1,-1]>=0 and beat_frame_interp[-1,-1]<n_frame):
        result[beat_frame_interp[-1,-1]]=(beat_pos[-1]-1)*beat_division+1
    return result
...
class FramedSubbeatAnnotation(ExtractorBase):
    ...
    def extract(self,entry,**kwargs):
        proxy_name='beat' if 'source' not in kwargs else kwargs['source']
        beat_division=kwargs['beat_division']
        result=get_framed_subbeat_annotation(entry,proxy_name,beat_division=beat_division)
        return result
